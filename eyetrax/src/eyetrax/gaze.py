...
class GazeEstimator:
    def __init__(
        ...
        self._neutral_face_center = None
    ...
    def extract_features(self, image):
        ...
        "aici"
        if self._neutral_face_center is None:
            self._neutral_face_center = eye_center.copy()
        self.movement_amount = float(
            np.linalg.norm(eye_center[:2] - self._neutral_face_center[:2])
                                    )
        self.patient_moved = self.movement_amount > self._movement_threshold
        "aici"
    ...
    def reset_movement_center(self):
        self._neutral_face_center = None
        self.patient_moved = False
        self.movement_amount = 0.0
